...
import { DashboardData, UpcomingReservation, RecentActivity, FavoriteSpace } from '../types/dashboard';
...
export const dashboardService = {
  ...
  getReservations: async (startDate: string, endDate: string): Promise<UpcomingReservation[]> => {
    try {
  ...
  getUpcomingReservations: async (limit?: number): Promise<UpcomingReservation[]> => {
    try {
      ...
      const upcoming = mockDashboardData.upcomingReservations
        .filter(reservation => new Date(`${reservation.date}T${reservation.startTime}`) > new Date());
  ...
  // Extend a booking
  extendBooking: async (reservationId: string, additionalHours: number): Promise<ExtendBookingResponse> => {
    try {
      ...
      return {
        success: true,
        message: `Booking extended by ${additionalHours} hour${additionalHours > 1 ? 's' : ''}`,
        updatedReservation: mockDashboardData.upcomingReservations.find(r => r.id === reservationId)
      };
    } catch (error) {
  ...
  // Cancel a booking
  cancelBooking: async (reservationId: string): Promise<CancelBookingResponse> => {
    try {
      ...
      const reservation = mockDashboardData.upcomingReservations.find(r => r.id === reservationId);
  ...
  // Modify a booking
  modifyBooking: async (reservationId: string, updates: Partial<UpcomingReservation>) => {
    try {
  ...
  // Get user activity
  getUserActivity: async (options: UserActivityOptions = {}): Promise<RecentActivity[]> => {
    try {
  ...
  addToFavorites: async (spaceId: string) => {
    try {
  ...
  removeFromFavorites: async (spaceId: string) => {
    try {
  ...
  getMonthlyStats: async (year: number = new Date().getFullYear()) => {
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 500));
      return mockDashboardData.monthlyBookings.filter(data => data.year === year);
    } catch (error) {
        ...
        recommendations: [
          {
            ...
            description: 'Book creative spaces during off-peak hours (2-4 PM) for better rates',
          ...
          },
          {
            ...
            description: 'You book most spaces between 9-11 AM. Consider extending these sessions.',
