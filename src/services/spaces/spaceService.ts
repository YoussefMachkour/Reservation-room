...
import type { Space, SpaceType, SpaceStatus, SpaceFilters } from '../../types/space';
...
const mockSpaces: Space[] = [
  {
    ...
    status: 'available',
    description: 'Large conference room with modern AV equipment perfect for team meetings and presentations.',
    surface: 45.5,
    photos: [
  ...
  },
  {
    ...
    status: 'available',
    description: 'Cozy meeting room perfect for small team discussions and one-on-ones.',
    surface: 25.0,
    photos: [
  ...
  },
  {
    ...
    room_number: 'AUD-01',
    equipment: [
      { name: 'Sound System', quantity: 1, description: 'Professional audio system' },
  ...
  },
  {
    ...
    status: 'available',
    description: 'Modern hot desk with dual monitors in an open workspace environment.',
    surface: 6.0,
    photos: [
  ...
  },
  {
    ...
    status: 'available',
    description: 'Private office suite with multiple workstations, perfect for small teams needing dedicated space.',
    surface: 80.0,
    photos: [
  ...
  },
  {
    ...
    status: 'available',
    description: 'Large open workspace perfect for collaborative work and team activities.',
    surface: 150.0,
    photos: [
      'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&h=600'
    ],
  ...
  },
  {
    ...
    room_number: '501',
    equipment: [
      { name: 'Conference Table', quantity: 1, description: 'Large mahogany table' },
      { name: 'Executive Chairs', quantity: 16 },
      { name: 'Video Conference System', quantity: 1, description: 'High-end system' },
      { name: 'Smart Board', quantity: 1 }
    ],
    status: 'maintenance',
    description: 'Premium executive conference room with high-end furnishings and technology.',
    surface: 60.0,
    photos: [
      'https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=800&h=600'
    ],
...
export class SpaceService {
  // Get all spaces
  static async getSpaces(filters?: SpaceFilters): Promise<Space[]> {
    try {
      ...
      const response = await apiClient.get<Space[]>(endpoint);
  ...
  static async createSpace(data: Omit<Space, 'id' | 'created_at' | 'updated_at'>): Promise<Space> {
    try {
  ...
  // Search spaces
  static async searchSpaces(query: string, filters?: SpaceFilters): Promise<Space[]> {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        if (query.trim()) {
          ...
          spaces = spaces.filter(space =>
            ...
            space.equipment.some(eq => eq.name.toLowerCase().includes(searchTerm))
          );
        }
      ...
      const response = await apiClient.get<Space[]>(`/spaces/search?${params.toString()}`);
  ...
  static async getAvailableSpaces(startTime: string, endTime: string, capacity?: number): Promise<Space[]> {
    try {
      ...
      const response = await apiClient.get<Space[]>(`/spaces/available?${params.toString()}`);
  ...
  // Get spaces by type
  static async getSpacesByType(type: SpaceType): Promise<Space[]> {
    try {
  ...
  static async getSpaceStats(): Promise<{
    ...
    reserved: number;
    by_type: Record<SpaceType, number>;
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        const stats = {
          ...
          out_of_service: mockSpaces.filter(s => s.status === 'out_of_service').length,
          reserved: mockSpaces.filter(s => s.status === 'reserved').length,
          by_type: {} as Record<SpaceType, number>,
        ...
        mockSpaces.forEach(space => {
          stats.by_building[space.building] = (stats.by_building[space.building] || 0) + 1;
        });
  ...
  private static applyFilters(spaces: Space[], filters: SpaceFilters): Space[] {
    ...
    if (filters.building) {
      filtered = filtered.filter(s => s.building.toLowerCase().includes(filters.building!.toLowerCase()));
    }
    ...
    if (filters.priceRange) {
      ...
      filtered = filtered.filter(s => s.price_per_hour >= min && s.price_per_hour <= max);
    }
...
export const getSpaceStatusLabel = (status: SpaceStatus): string => {
  const labels = {
    available: 'Available',
...
export const getSpaceStatusColor = (status: SpaceStatus): string => {
  const colors = {
