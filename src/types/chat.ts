...
export class ChatUtils {
  ...
  static generateSmartResponse(userMessage: string): string {
    ...
    if (message.includes('technical') || message.includes('problem') || message.includes('issue')) {
  ...
  static getMessageTypeColor(type: MessageType, isDark: boolean = false): string {
    const darkClasses = isDark ? 'dark:bg-opacity-20 dark:border-opacity-60' : '';
    
    switch (type) {
      case 'booking_confirmation': 
        return `bg-green-50 border-green-200 text-green-800 ${isDark ? 'dark:bg-green-900/20 dark:border-green-800 dark:text-green-300' : ''}`;
      case 'membership_renewal': 
        return `bg-blue-50 border-blue-200 text-blue-800 ${isDark ? 'dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-300' : ''}`;
      case 'cancellation': 
        return `bg-red-50 border-red-200 text-red-800 ${isDark ? 'dark:bg-red-900/20 dark:border-red-800 dark:text-red-300' : ''}`;
      case 'payment_reminder': 
        return `bg-yellow-50 border-yellow-200 text-yellow-800 ${isDark ? 'dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-300' : ''}`;
      case 'system_notification': 
        return `bg-purple-50 border-purple-200 text-purple-800 ${isDark ? 'dark:bg-purple-900/20 dark:border-purple-800 dark:text-purple-300' : ''}`;
      default: 
        return '';
    }
  }

  static getAgentStatusColor(status: SupportAgent['status'], isDark: boolean = false): string {
    switch (status) {
      case 'online': 
  ...
  static getConnectionStatusColor(status: 'connected' | 'connecting' | 'disconnected', isDark: boolean = false): string {
    switch (status) {
      case 'connected': 
        return `text-green-600 ${isDark ? 'dark:text-green-400' : ''}`;
      case 'connecting': 
