      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Here's what's happening in your spaces today - {currentTime.toLocaleDateString()}
          </p>
        </div>
            ...
            <Card key={index} className="hover:shadow-md transition-shadow cursor-pointer">
              ...
              <CardContent>
                ...
                <div className="flex items-center text-xs text-muted-foreground">
                  <TrendIcon className={`mr-1 h-3 w-3 ${stat.trend === 'up' ? 'text-green-600' : 'text-red-600'}`} />
                  {stat.change}
                </div>
                ...
                <div className="mt-2 flex items-end space-x-1 h-6">
                  {stat.data.map((value, i) => (
                    <div
                      key={i}
                      className={`w-2 bg-${stat.color.split('-')[1]}-200 rounded-sm`}
              ...
              {recentActivities.map((activity) => (
                <div key={activity.id} className="flex items-center space-x-4 p-2 rounded-lg hover:bg-muted/50">
                  ...
                  <Badge variant={
                    ...
                    activity.type === 'membership' ? 'secondary' : 
                    activity.type === 'payment' ? 'outline' :
                    'destructive'
                  }>
                    {activity.type}
                  </Badge>
