...
export const SpaceBooking: React.FC = () => {
  ...
  useEffect(() => {
    if (space) {
      const favorites = JSON.parse(
        localStorage.getItem("space-favorites") || "[]"
      );
  ...
  const loadSpace = async () => {
    ...
    try {
      ...
      setSpace(spaceData.data!); // Extract the Space object from the spaceData object
    } catch (err: any) {
  ...
  const loadUpcomingReservations = async () => {
    ...
    try {
      ...
      const reservationsData = await BookingService.getSpaceReservations(
        spaceId,
        startDate,
        endDate
      );
      ...
      <div className="flex items-center mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {getSpaceTypeLabel(space.type)} • {space.building} - Floor{" "}
            {space.floor} - Room {space.room_number}
          </p>
        </div>
        <div className="flex items-center space-x-2">
              ...
              {space.requires_approval && (
                <div className="flex items-start space-x-3">
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    This space requires manager approval before your booking is
                    confirmed.
                  </p>
                </div>
              )}
