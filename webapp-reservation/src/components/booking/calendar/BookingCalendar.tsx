...
import { formatDuration, getReservationDuration, getStatusBgColor } from '../../../utils/bookingHelpers';
        ...
        {calendarDays.map((day, index) => (
          <div
            ...
            className={`border-r border-b border-gray-200 dark:border-gray-700 ${
              ...
              onDateSelect ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700' : ''
            }`}
          ...
          >
            <div className="p-2 h-full flex flex-col">
              ...
              <div className="flex-1 space-y-1 overflow-hidden">
                {getReservationsByTimeSlot(day.reservations).map((reservation, idx) => {
                  const duration = getReservationDuration(reservation.start_time, reservation.end_time);
                  
                  return (
                    <div
                      key={reservation.id}
                      className={`text-xs p-1 rounded cursor-pointer hover:opacity-80 transition-opacity ${getStatusBgColor(reservation.status)}`}
                      onClick={(e) => handleReservationClick(e, reservation)}
                      title={`${reservation.title} - ${format(parseISO(reservation.start_time), 'h:mm a')} (${formatDuration(duration)})`}
                    >
