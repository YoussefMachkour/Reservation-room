...
export const formatDateRange = (startTime: string, endTime: string): string => {
  ...
  if (startDate === endDate) {
    return `${startDate} from ${format(start, "h:mm a")} to ${format(
      end,
      "h:mm a"
    )}`;
  }
...
export const validateBookingTimes = (
  ...
  if (duration > maxDuration) {
    return {
      valid: false,
      error: `Booking duration cannot exceed ${formatDuration(maxDuration)}`,
    };
  }
...
export const validateParticipantCount = (
  ...
  if (count > spaceCapacity) {
    return {
      valid: false,
      error: `Maximum capacity is ${spaceCapacity} people`,
    };
  }
...
export const sortReservationsByTime = (
...
export const hasEquipment = (
  space: { equipment?: Array<{ name: string }> },
...
export const getTotalEquipmentCount = (
